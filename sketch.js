...
//quiz
quiz=createSprite(windowWidth/2,windowHeight/2)
quiz.addImage(qu)
quiz.scale=1.5
quiz.visible=false
...
submit.position(windowWidth/2.19,500)
submit.hide()
 ...
 if(em.collide(ld)){ 
   ...
   com.shapeColor='lightgreen'
    ...
    if(com.shapeColor='lightgreen'){
      ...
      but.position(windowWidth/2,620)
      but.mousePressed(()=>{
         location.reload()
...
textSize(100)
fill('orange')
text ('🤑:'+coins,windowWidth/2,100)
text ('❤️:'+lives,windowWidth/2,310)
textSize(130)
fill('yellow')
